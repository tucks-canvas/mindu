...
const preferences = [
  { 
    id: 1,
    title: 'Journal Tracking Features',
    icon: 'lswitch',
  },
  { 
    id: 2,
    title: 'Goal Tracking Features',
    icon: 'lswitch',
  },
  { 
    id: 3,
    title: 'Community Chat Features',
    icon: 'lswitch',
  },
  {  
    id: 4,
    title: 'Hide Name (To Friends)',
    icon: 'lswitch',
  },
  { 
    id: 5,
    title: 'Hide Name (To Everyone)',
    icon: 'lswitch',
  },
];
...
const Preferences = () => {
  ...
  const [preferenceSettings, setPreferenceSettings] = useState<typeof preferences>(preferences);

  // Load preference states from AsyncStorage
  useEffect(() => {
    const loadSettings = async () => {
      const savedSettings = await AsyncStorage.getItem("preferenceSettings");
      if (savedSettings) {
        setPreferenceSettings(JSON.parse(savedSettings));
      }
    };
    loadSettings();
  }, []);

  // Save preference states to AsyncStorage
  ...
  const togglePreference = (id) => {
    const updatedSettings = preferenceSettings.map((preference) =>
      preference.id === id
        ? {
            ...preference,
            icon:
              preference.icon === "lswitch"
                ? "rswitch"
                : "lswitch",
          }
        : preference
          ...
          <View style={styles.container1}>
            ...
            <View style={styles.container2}>              
              ...
              <View style={styles.body}>
                ...
                {preferenceSettings.map((preference) => (
                  <TouchableOpacity key={preference.id}>
